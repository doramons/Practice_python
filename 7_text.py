...
character_y_pos = screen_height/2 - character_height/2 # 화면 세로 크기 가장 아래에 해당
...
# 적 enemy 캐릭터
enemy = pygame.image.load("C:/Users/Playdata/Desktop/파이썬활용/retrogame/enemy.png")
enemy_size = enemy.get_rect().size # 이미지의 크기 구하기
enemy_width = enemy_size[0] # 캐릭터 가로크기
enemy_height = enemy_size[1] # 캐릭터 세로크기
enemy_x_pos = (screen_width - enemy_width)/2 # 화면 가로 절반 크기
enemy_y_pos = screen_height/2 - enemy_height/2 # 화면 세로 크기 가장 아래에 해당
...
while running:
    ...
    # 경과시간을 (ms) 1000으로 나누어서 초(s) 단위로 표시
